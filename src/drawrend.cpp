...
{
  return - (p1x - p2x) * (p3y - p2y) + (p3x - p2x) * (p1y - p2y);
}
bool insideTri(float p1x, float p1y, float p2x, float p2y, float p3x, float p3y, float p4x, float p4y)
{
  bool b1, b2, b3;
  b1 = sign(p1x, p1y, p2x, p2y, p3x, p3y) >= 0;
  b2 = sign(p1x, p1y, p3x, p3y, p4x, p4y) >= 0;
  b3 = sign(p1x, p1y, p4x, p4y, p2x, p2y) >= 0;
...
void DrawRend::rasterize_triangle( float x0, float y0,
  ...
  int samplebuffer_size = samplebuffer.size();

  // part 6 update: 
  Vector3D bary_coo, dx_bary, dy_bary;

  float sub_pixel_length = 1 / sqrt(sample_rate);
  int sub_pixel_side = sqrt(sample_rate);
  float x = 0; float y = 0;

  for (int i = min_y; i < max_y; ++i) {
    for (int j = min_x; j < max_x; ++j) {
      // if (min_y >= 0 && min_x >= 0 && max_y < samplebuffer_size && max_x < samplebuffer[i].size()) {

        for (int k = 0; k < sub_pixel_side; ++k) {
          for (int t = 0; t < sub_pixel_side; ++t) {
            x = j + k * sub_pixel_length + sub_pixel_length / 2.0;
            y = i + t * sub_pixel_length + sub_pixel_length / 2.0;

            if (insideTri(x, y, x0, y0, x1, y1, x2, y2)) {
              if (tri == NULL) {
                samplebuffer[i][j].fill_color(k, t, color);
              }
              else {
                // bary
                bary_coo = barycentric(x, y, x0, y0, x1, y1, x2, y2);
                // (x+1,y)
                dx_bary = barycentric(x+1, y, x0, y0, x1, y1, x2, y2);
                // (x,y+1)
                dy_bary = barycentric(x, y+1, x0, y0, x1, y1, x2, y2);

                SampleParams sp = SampleParams();
                sp.lsm = lsm;
                sp.psm = psm;
                samplebuffer[i][j].fill_color(k, t, tri->color(bary_coo, dx_bary, dy_bary, sp));
              }
            }
          }
        }
